...
const Products = ({ products, gap }: ProductsProps) => {
  ...
  const formatAsYen = useMemo(() => {
    ...
    return (amount: number): string => yenFormatter.format(amount);
  }, []);
        ...
        <nav className=" text-sm font-normal sm:font-medium">
          ...
          <div className=" flex gap-3">
            <span className=" text-sm text-lightGray line-through ">
               {formatAsYen(products.oldPrice)}
            </span>
            <b className=" text-zinc-900 "> {formatAsYen(products.price)} </b>
          </div>
        </nav>
